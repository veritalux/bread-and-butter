...
const OREGON_DEPENDENT_DEDUCTION = 1135;
...
export function estimateTaxRate(monthlyIncome: number, isDependent = false): number {
  ...
  // Dependents use a reduced standard deduction: max($1,300, min(earned income + $450, $14,600))
  const federalDeduction = isDependent
    ? Math.max(FEDERAL_DEPENDENT_MIN_DEDUCTION, Math.min(annualIncome + 450, FEDERAL_STANDARD_DEDUCTION))
