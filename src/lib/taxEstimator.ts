...
const FEDERAL_DEPENDENT_MIN_DEDUCTION = 1300; // 2024 minimum for dependents
...
const OREGON_DEPENDENT_MIN_DEDUCTION = 1135; // 2024 minimum for dependents
...
export function estimateTaxRate(monthlyIncome: number, isDependent = false): number {
  ...
  if (isDependent) {
    // Dependent standard deduction: greater of minimum or (earned income + $450), capped at regular deduction
    federalDeduction = Math.min(
      Math.max(FEDERAL_DEPENDENT_MIN_DEDUCTION, annualIncome + 450),
      FEDERAL_STANDARD_DEDUCTION
    );
    oregonDeduction = Math.min(
      Math.max(OREGON_DEPENDENT_MIN_DEDUCTION, annualIncome),
