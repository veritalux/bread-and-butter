...
// Uses 2024/2025 brackets. Supports standard filer and dependent filer.
...
// Dependents have a limited standard deduction: max($1,300, min(earned income + $450, regular deduction))
function getFederalDeduction(annualIncome: number, isDependent: boolean): number {
  if (!isDependent) return FEDERAL_STANDARD_DEDUCTION;
  return Math.max(1300, Math.min(annualIncome + 450, FEDERAL_STANDARD_DEDUCTION));
}

// Oregon mirrors the federal dependent rule: deduction is limited to lesser of regular amount or earned income
function getOregonDeduction(annualIncome: number, isDependent: boolean): number {
  if (!isDependent) return OREGON_STANDARD_DEDUCTION;
  return Math.min(OREGON_STANDARD_DEDUCTION, annualIncome);
}
...
export function estimateTaxRate(monthlyIncome: number, isDependent = false): number {
  ...
  const federalTaxable = Math.max(0, annualIncome - getFederalDeduction(annualIncome, isDependent));
  const federalTax = calcProgressiveTax(federalTaxable, FEDERAL_BRACKETS);

  const oregonTaxable = Math.max(0, annualIncome - getOregonDeduction(annualIncome, isDependent));
