...
// Uses 2024/2025 brackets. Single filer. Adjusts standard deductions for dependents.

const FEDERAL_STANDARD_DEDUCTION = 14600;
// Dependents: max($1,300, min(earned_income + $450, standard deduction))
const FEDERAL_DEPENDENT_MIN_DEDUCTION = 1300;
...
// Oregon does not allow a standard deduction for dependents who can be claimed on another return.
...
export function estimateTaxRate(monthlyIncome: number, isDependent = false): number {
  ...
  const federalStandardDeduction = isDependent
    ? Math.max(
        FEDERAL_DEPENDENT_MIN_DEDUCTION,
        Math.min(annualIncome + FEDERAL_DEPENDENT_EARNED_BONUS, FEDERAL_STANDARD_DEDUCTION)
      )
    : FEDERAL_STANDARD_DEDUCTION;

  const oregonStandardDeduction = isDependent ? 0 : OREGON_STANDARD_DEDUCTION;

  const federalTaxable = Math.max(0, annualIncome - federalStandardDeduction);
  const federalTax = calcProgressiveTax(federalTaxable, FEDERAL_BRACKETS);

  const oregonTaxable = Math.max(0, annualIncome - oregonStandardDeduction);
