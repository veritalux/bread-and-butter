...
// If isDependent is true, uses dependent-specific deduction limits:
// Federal: max($1,300, min($14,600, earned income + $450))
// Oregon: $0 (dependents cannot claim Oregon standard deduction)
export function estimateTaxRate(monthlyIncome: number, isDependent = false): number {
  ...
  if (isDependent) {
    federalDeduction = Math.max(1300, Math.min(FEDERAL_STANDARD_DEDUCTION, annualIncome + 450));
    oregonDeduction = 0;
  } else {
