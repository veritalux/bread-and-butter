...
// Dependent standard deduction: greater of $1,300 or (earned income + $450), capped at $14,600
...
const OREGON_DEPENDENT_STANDARD_DEDUCTION = 1145;
...
export function estimateTaxRate(monthlyIncome: number, isDependent = false): number {
  ...
  const federalDeduction = isDependent
    ? Math.min(
        ...
        Math.max(FEDERAL_DEPENDENT_MIN_DEDUCTION, annualIncome + FEDERAL_DEPENDENT_EARNED_BONUS)
      )
    : FEDERAL_STANDARD_DEDUCTION;
