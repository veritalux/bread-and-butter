...
// Uses 2024/2025 brackets. Single filer. Dependent flag reduces standard deductions
// per IRS rules: dependent deduction = max($1,300, earned income + $450), capped at $14,600.
...
const OREGON_DEPENDENT_DEDUCTION_EARNED_BONUS = 300;
...
export function estimateTaxRate(monthlyIncome: number, isDependent = false): number {
  ...
  if (isDependent) {
    ...
    oregonDeduction = Math.min(
      Math.max(OREGON_DEPENDENT_DEDUCTION_MIN, annualIncome + OREGON_DEPENDENT_DEDUCTION_EARNED_BONUS),
