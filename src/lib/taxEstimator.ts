...
// When isDependent=true, applies the reduced dependent standard deduction:
//   min($14,600, max($1,300, earned income + $450))

const FEDERAL_STANDARD_DEDUCTION = 14600;
const FEDERAL_DEPENDENT_DEDUCTION_MIN = 1300;
const FEDERAL_DEPENDENT_EARNED_EXTRA = 450;
...
export function estimateTaxRate(monthlyIncome: number, isDependent = false): number {
  ...
  const federalDeduction = isDependent
    ? Math.min(FEDERAL_STANDARD_DEDUCTION, Math.max(FEDERAL_DEPENDENT_DEDUCTION_MIN, annualIncome + FEDERAL_DEPENDENT_EARNED_EXTRA))
