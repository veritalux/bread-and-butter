...
// For dependents, deduction is the greater of $1,300 or (earned income + $450), capped at the standard deduction.
...
const OREGON_DEPENDENT_DEDUCTION = 2350;
...
export function estimateTaxRate(monthlyIncome: number, isDependent = false): number {
  ...
  const federalDeduction = isDependent
    ? Math.min(FEDERAL_STANDARD_DEDUCTION, Math.max(FEDERAL_DEPENDENT_MIN_DEDUCTION, annualIncome + FEDERAL_DEPENDENT_EARNED_ADDITION))
