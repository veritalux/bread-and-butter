...
// Dependent standard deduction: greater of $1,300 or (earned income + $450), capped at regular deduction
...
const OREGON_DEPENDENT_DEDUCTION = 1145;
...
export function estimateTaxRate(monthlyIncome: number, isDependent = false): number {
  ...
  const federalDeduction = isDependent
    ? Math.min(
        ...
        Math.max(FEDERAL_DEPENDENT_DEDUCTION_BASE, annualIncome + FEDERAL_DEPENDENT_EARNED_ADDITION)
      )
    : FEDERAL_STANDARD_DEDUCTION;
