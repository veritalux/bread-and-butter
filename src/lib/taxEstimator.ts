...
// Uses 2024/2025 brackets. Single filer. Dependent status affects standard deductions.

const FEDERAL_STANDARD_DEDUCTION = 14600;
// Dependents: greater of $1,300 or (earned income + $450), capped at full deduction
...
// Oregon dependent standard deduction is a fixed reduced amount
const OREGON_DEPENDENT_DEDUCTION = 1115;
...
export function estimateTaxRate(monthlyIncome: number, isDependent = false): number {
  ...
  const federalDeduction = isDependent
    ? Math.min(
        Math.max(FEDERAL_DEPENDENT_DEDUCTION_MIN, annualIncome + FEDERAL_DEPENDENT_EARNED_BONUS),
