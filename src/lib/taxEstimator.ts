...
// Dependents get a reduced federal standard deduction and no Oregon standard deduction.
...
export function estimateTaxRate(monthlyIncome: number, isDependent = false): number {
  ...
  // Dependents: federal deduction = max($1,300, earned income + $450), capped at standard deduction.
  // Dependents cannot claim Oregon standard deduction.
  const federalDeduction = isDependent
    ? Math.min(Math.max(1300, annualIncome + 450), FEDERAL_STANDARD_DEDUCTION)
