...
const FEDERAL_STANDARD_DEDUCTION_SINGLE = 14600;
const FEDERAL_STANDARD_DEDUCTION_DEPENDENT_MIN = 1300;
...
// Oregon standard deduction: single filer $2,745; dependents use a reduced amount (~$1,100)
const OREGON_STANDARD_DEDUCTION_SINGLE = 2745;
const OREGON_STANDARD_DEDUCTION_DEPENDENT = 1100;
...
export function estimateTaxRate(monthlyIncome: number, isDependent = false): number {
  ...
  const federalDeduction = isDependent
    ? Math.min(
        Math.max(FEDERAL_STANDARD_DEDUCTION_DEPENDENT_MIN, annualIncome + FEDERAL_STANDARD_DEDUCTION_DEPENDENT_EARNED_BONUS),
  ...
  const oregonDeduction = isDependent
    ...
    : OREGON_STANDARD_DEDUCTION_SINGLE;
