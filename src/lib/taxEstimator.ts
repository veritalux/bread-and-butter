...
// Uses 2024/2025 brackets. Adjusts standard deductions for dependents.

const FEDERAL_STANDARD_DEDUCTION = 14600;
const FEDERAL_DEPENDENT_DEDUCTION_MIN = 1300;
const FEDERAL_DEPENDENT_DEDUCTION_BONUS = 450;

const OREGON_STANDARD_DEDUCTION = 2745;
const OREGON_DEPENDENT_DEDUCTION_MIN = 1215;
const OREGON_DEPENDENT_DEDUCTION_BONUS = 420;
...
function dependentDeduction(annualIncome: number, minDeduction: number, bonus: number, maxDeduction: number): number {
  // IRS rule: greater of minimum or (earned income + bonus), capped at the regular deduction
  return Math.max(minDeduction, Math.min(annualIncome + bonus, maxDeduction));
}
...
export function estimateTaxRate(monthlyIncome: number, isDependent = false): number {
  ...
  const federalDeduction = isDependent
    ? dependentDeduction(annualIncome, FEDERAL_DEPENDENT_DEDUCTION_MIN, FEDERAL_DEPENDENT_DEDUCTION_BONUS, FEDERAL_STANDARD_DEDUCTION)
    : FEDERAL_STANDARD_DEDUCTION;

  const oregonDeduction = isDependent
    ? dependentDeduction(annualIncome, OREGON_DEPENDENT_DEDUCTION_MIN, OREGON_DEPENDENT_DEDUCTION_BONUS, OREGON_STANDARD_DEDUCTION)
