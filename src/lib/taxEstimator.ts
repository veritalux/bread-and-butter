...
const OREGON_DEPENDENT_MIN_DEDUCTION = 1265;
...
export function estimateTaxRate(monthlyIncome: number, isDependent = false): number {
  ...
  // Dependents have a limited standard deduction: max(min_deduction, earned_income + $450)
  ...
  const oregonDeduction = isDependent
    ? Math.min(OREGON_STANDARD_DEDUCTION, Math.max(OREGON_DEPENDENT_MIN_DEDUCTION, annualIncome + 450))
