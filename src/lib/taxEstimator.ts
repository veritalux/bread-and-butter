...
// Uses 2024/2025 brackets. Supports dependent filers (reduced standard deduction).

const FEDERAL_STANDARD_DEDUCTION = 14600;
// Dependents: greater of $1,300 or (earned income + $450), capped at full standard deduction
const FEDERAL_DEPENDENT_DEDUCTION_MIN = 1300;
const FEDERAL_DEPENDENT_DEDUCTION_EARNED_BONUS = 450;
...
// Oregon dependents: greater of $1,135 or (earned income + $350), capped at full standard deduction
const OREGON_DEPENDENT_DEDUCTION_MIN = 1135;
const OREGON_DEPENDENT_DEDUCTION_EARNED_BONUS = 350;
...
export function estimateTaxRate(monthlyIncome: number, isDependent = false): number {
  ...
  const federalDeduction = isDependent
    ? Math.min(FEDERAL_STANDARD_DEDUCTION, Math.max(FEDERAL_DEPENDENT_DEDUCTION_MIN, annualIncome + FEDERAL_DEPENDENT_DEDUCTION_EARNED_BONUS))
    : FEDERAL_STANDARD_DEDUCTION;

  const oregonDeduction = isDependent
    ? Math.min(OREGON_STANDARD_DEDUCTION, Math.max(OREGON_DEPENDENT_DEDUCTION_MIN, annualIncome + OREGON_DEPENDENT_DEDUCTION_EARNED_BONUS))
