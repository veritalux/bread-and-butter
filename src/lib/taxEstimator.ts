...
// Uses 2024/2025 brackets. Supports single filer or dependent filer.

const FEDERAL_STANDARD_DEDUCTION = 14600;
// Dependents: greater of $1,300 or (earned income + $450), capped at regular deduction
const FEDERAL_DEPENDENT_BASE = 1300;
...
// Oregon dependent standard deduction: greater of $1,270 or (earned income + $350), capped at regular
const OREGON_DEPENDENT_BASE = 1270;
...
export function estimateTaxRate(monthlyIncome: number, isDependent = false): number {
  ...
  let federalDeduction = FEDERAL_STANDARD_DEDUCTION;
  let oregonDeduction = OREGON_STANDARD_DEDUCTION;

  if (isDependent) {
    federalDeduction = Math.min(
      FEDERAL_STANDARD_DEDUCTION,
      Math.max(FEDERAL_DEPENDENT_BASE, annualIncome + FEDERAL_DEPENDENT_EARNED_BONUS)
    );
    oregonDeduction = Math.min(
      OREGON_STANDARD_DEDUCTION,
      Math.max(OREGON_DEPENDENT_BASE, annualIncome + OREGON_DEPENDENT_EARNED_BONUS)
    );
  }
