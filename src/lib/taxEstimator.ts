...
// Dependent federal deduction: max($1,300, min($14,600, earnedIncome + $450))
const FEDERAL_DEPENDENT_DEDUCTION_MIN = 1300;
const FEDERAL_DEPENDENT_DEDUCTION_ADDON = 450;

const OREGON_STANDARD_DEDUCTION = 2745;
const OREGON_DEPENDENT_STANDARD_DEDUCTION = 1240;
...
export function estimateTaxRate(monthlyIncome: number, isDependent = false): number {
  ...
  const federalDeduction = isDependent
    ? Math.max(FEDERAL_DEPENDENT_DEDUCTION_MIN, Math.min(FEDERAL_STANDARD_DEDUCTION, annualIncome + FEDERAL_DEPENDENT_DEDUCTION_ADDON))
