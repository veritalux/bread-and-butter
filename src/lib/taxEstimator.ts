...
// Uses 2024/2025 brackets. Handles dependent vs. independent filer standard deductions.

const FEDERAL_STANDARD_DEDUCTION = 14600;
// Dependent federal standard deduction: greater of $1,300 or earned income + $450, capped at $14,600
const FEDERAL_DEPENDENT_MIN = 1300;
const FEDERAL_DEPENDENT_EARNED_ADDITION = 450;
...
// Dependent Oregon standard deduction: greater of $1,135 or earned income + $350, capped at $2,745
const OREGON_DEPENDENT_MIN = 1135;
const OREGON_DEPENDENT_EARNED_ADDITION = 350;
...
export function estimateTaxRate(monthlyIncome: number, isDependent = false): number {
  ...
  const federalDeduction = isDependent
    ? Math.min(FEDERAL_STANDARD_DEDUCTION, Math.max(FEDERAL_DEPENDENT_MIN, annualIncome + FEDERAL_DEPENDENT_EARNED_ADDITION))
    : FEDERAL_STANDARD_DEDUCTION;

  const oregonDeduction = isDependent
    ? Math.min(OREGON_STANDARD_DEDUCTION, Math.max(OREGON_DEPENDENT_MIN, annualIncome + OREGON_DEPENDENT_EARNED_ADDITION))
