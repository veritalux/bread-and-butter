...
// Dependents have reduced standard deductions per IRS Publication 501 and Oregon OR-17.

const FEDERAL_STANDARD_DEDUCTION = 14600;
// Dependent federal std deduction: greater of $1,300 or (earned income + $450), capped at $14,600
const FEDERAL_DEPENDENT_MIN_DEDUCTION = 1300;
const FEDERAL_DEPENDENT_INCOME_ADDER = 450;
...
const OREGON_DEPENDENT_DEDUCTION = 1205;
...
export function estimateTaxRate(monthlyIncome: number, isDependent = false): number {
  ...
  const federalDeduction = isDependent
    ? Math.min(
        Math.max(FEDERAL_DEPENDENT_MIN_DEDUCTION, annualIncome + FEDERAL_DEPENDENT_INCOME_ADDER),
