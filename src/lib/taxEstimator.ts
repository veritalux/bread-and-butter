...
const FEDERAL_DEPENDENT_MIN_DEDUCTION = 1300; // 2024 dependent standard deduction floor
...
const OREGON_DEPENDENT_DEDUCTION = 1145; // 2024 Oregon dependent standard deduction
...
export function estimateTaxRate(monthlyIncome: number, isDependent = false): number {
  ...
  // Dependents have a limited standard deduction: greater of $1,300 or (earned income + $450), capped at regular deduction
  const federalDeduction = isDependent
    ? Math.min(FEDERAL_STANDARD_DEDUCTION, Math.max(FEDERAL_DEPENDENT_MIN_DEDUCTION, annualIncome + 450))
    : FEDERAL_STANDARD_DEDUCTION;

  const oregonDeduction = isDependent ? OREGON_DEPENDENT_DEDUCTION : OREGON_STANDARD_DEDUCTION;
