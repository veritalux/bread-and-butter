...
// Dependent filers: limited to greater of $1,300 or (earned income + $450), capped at regular deduction
...
// Oregon dependent filers: limited to greater of $1,315 or (earned income + $450), capped at regular deduction
const OREGON_DEPENDENT_DEDUCTION_MIN = 1315;
...
export function estimateTaxRate(monthlyIncome: number, isDependent = false): number {
  ...
  if (isDependent) {
    // Dependent standard deduction is limited
    federalDeduction = Math.min(
      Math.max(FEDERAL_DEPENDENT_DEDUCTION_MIN, annualIncome + FEDERAL_DEPENDENT_DEDUCTION_EARNED_BONUS),
      FEDERAL_STANDARD_DEDUCTION
    );
    oregonDeduction = Math.min(
      Math.max(OREGON_DEPENDENT_DEDUCTION_MIN, annualIncome + FEDERAL_DEPENDENT_DEDUCTION_EARNED_BONUS),
