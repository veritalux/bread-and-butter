...
// Uses 2024/2025 brackets. Supports single filer (dependent or independent).
...
const OREGON_DEPENDENT_DEDUCTION = 1100;
...
export function estimateTaxRate(monthlyIncome: number, isDependent = false): number {
  ...
  const federalDeduction = isDependent
    ? Math.min(FEDERAL_STANDARD_DEDUCTION, Math.max(FEDERAL_DEPENDENT_DEDUCTION_MIN, annualIncome + FEDERAL_DEPENDENT_DEDUCTION_BONUS))
