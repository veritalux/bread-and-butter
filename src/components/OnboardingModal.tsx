            ...
            {step === 4 && (
              <div>
                ...
                <p className="text-xs text-[var(--color-text-muted)] mb-4">
                  This helps us accurately estimate your tax rate. You're typically a dependent if a parent or guardian claims you on their tax return.
                </p>
                <div className="grid grid-cols-2 gap-3">
                  {[
                    { label: "Yes, I'm a dependent", value: true },
                    { label: "No, I file independently", value: false },
                  ].map(({ label, value }) => (
                    <button
                      key={String(value)}
                      type="button"
                      onClick={() => setIsDependent(value)}
                      className={`flex items-center justify-center gap-2 px-4 py-4 rounded-lg border text-sm font-medium text-center transition-all cursor-pointer ${
                        isDependent === value
                    ...
                    >
                      <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${
                        ...
                        {isDependent === value && <Check size={10} className="text-[var(--color-primary-foreground)]" />}
                      </div>
                      {label}
                    </button>
                  ))}
