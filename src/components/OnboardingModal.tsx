...
export default function OnboardingModal({ onComplete }: Props) {
  ...
  const handleSubmit = () => {
    ...
    onComplete({
      ...
      isDependent: dep,
      taxRate: estimateTaxRate(income, dep),
            ...
            {step === 4 && (
              <div>
                ...
                <p className="text-xs text-[var(--color-text-muted)] mb-4">
                  This affects your standard deduction and changes how we estimate your tax rate.
                </p>
                <div className="flex flex-col gap-3">
                  {[
                    { value: false, label: "No — I file independently" },
                    { value: true, label: "Yes — someone claims me as a dependent" },
                  ].map(({ value, label }) => (
                    <button
                      key={String(value)}
                      type="button"
                    ...
                    >
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${
                        ...
                        {isDependent === value && <Check size={12} className="text-[var(--color-primary-foreground)]" />}
                      </div>
                      {label}
                    </button>
                  ))}
