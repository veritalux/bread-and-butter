    ...
    <main className="max-w-2xl mx-auto px-4 py-10">
      ...
      <section className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl p-6 mb-6">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-xs font-medium text-[var(--color-text-heading)] mb-1.5">Dependent status</label>
            <button
              type="button"
              onClick={() => { setIsDependent((v) => !v); setSaved(false); }}
            ...
            >
              <span>{isDependent ? "Claimed as dependent" : "Filing independently"}</span>
              <span className={`text-xs px-2 py-0.5 rounded-full ${isDependent ? "bg-[var(--color-primary)] text-[var(--color-primary-foreground)]" : "bg-[var(--color-border)] text-[var(--color-text-muted)]"}`}>
                {isDependent ? "Yes" : "No"}
              </span>
            </button>
            <p className="text-xs text-[var(--color-text-muted)] mt-1">Claimed on a parent or guardian's tax return</p>
          </div>
          ...
          <div>
            ...
            <p className="text-xs text-[var(--color-text-muted)] mt-1">Auto-calculated for Salem, Oregon</p>
          </div>
